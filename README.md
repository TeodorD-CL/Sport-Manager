...
User Booking Flow
The user opens the homepage at `/`.
The user searches or filters facilities by city, court type, date, amenities, rating, reviews, or price.
The user opens a facility page.
The facility page shows courts and one-hour availability slots based on the facility schedule.
Past slots are shown as unavailable.
The user selects an available slot.
If the user is not logged in, booking confirmation redirects to login.
The user can select suitable rentals for the selected sport type.
The app calculates the total price.
The user confirms the booking.
The booking is created with status `confirmed` and a UUID QR code value.
